...
$invalidPrefixes = array('Translations:', 'Draft:', 'Help:', 'File:'); // Add more invalid terms if needed.
...
function getGithubFileContent($filePath) {
    ...
    if ($httpcode == 200) {
        ...
        return base64_decode($content['content']);  // Decode the Base64 content.
    } else {
        return null;
    }
}
...
// Function to get the SHA of an existing file on GitHub
function getShaForFile($filePath) {
    global $githubRepoPath, $githubToken;
    $githubApiUrl = "https://api.github.com/repos/$githubRepoPath/contents/" . $filePath;
    $ch = curl_init($githubApiUrl);
    ...
    if ($httpcode == 200) {
        $content = json_decode($response, true);
        return $content['sha'];  // Return the SHA value
    } else {
        echo "ERROR: Could not fetch SHA for '$filePath'. HTTP Code: $httpcode.<br>";
        return null;
    }
}
...
if (isset($recentChanges['query']['recentchanges'])) {
    ...
    foreach ($recentPages as $page) {
        ...
        // Fetch the SHA for the existing file
        $sha = getShaForFile($filePath);

        // Skip if the content is already up-to-date
        if ($existingContent !== null && $wikiText === $existingContent) {
        ...
        if ($existingContent !== null) {
            ...
            updateGithubFile($filePath, $wikiText, $sha, $pageTitle, $user, $comment);
        } else {
